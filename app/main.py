...
def login():
    ...
    scope = 'user-read-private user-read-email user-top-read user-follow-read playlist-modify-private'
    params = {
        'client_id' : CLIENT_ID,
        'response_type' : 'code',
        'scope' : scope,
...
def top_3_playlist():
    ...
    tracks = [request.form.get('1-track'), request.form.get('2-track'), request.form.get('3-track')]

    playlist_data = {
        "name" : f'My TOP 3 Songs @{USERNAME} [{TIME_RANGE}]',
        "description" : f'The most played songs on {TIME_RANGE}, generated by Spotify-api-resume @fungirox',
        "public" : False
    }

    PLAYLIST_ID = create_playlist(headers=headers,USER_ID=USER_ID,DATA=playlist_data)
    if PLAYLIST_ID:
        JSON = {
            "uris": tracks
        }
        add_item_playlist(headers=headers,PLAYLIST_ID=PLAYLIST_ID,JSON=JSON)
        webbrowser.open(f'https://open.spotify.com/playlist/{PLAYLIST_ID}')
    
    return redirect('/spotify-cv')

def create_playlist(headers,USER_ID,DATA):
    id = ""
    try:
        response = requests.post(API_URL + f'users/{USER_ID}/playlists', headers=headers, json=DATA)
        response.raise_for_status()  
        id = response.json()['id']
    except Exception as e:
        print(f"Error creado playlist: {e}")
        return None
    return id

def add_item_playlist(headers,PLAYLIST_ID,JSON):
    id = ""
    try:
        response = requests.post(API_URL + f'playlists/{PLAYLIST_ID}/tracks', headers=headers, json=JSON)
        response.raise_for_status()  
        id = response.json()['snapshot_id']
    except Exception as e:
        print(f"Error añadiendo item a playlist: {e}")
        return None
    return id
